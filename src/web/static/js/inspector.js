...
function switchTab(name, btn) {
  currentTab = name;
  document
    .querySelectorAll("#detail-tabs .nav-link")
...
function populateRequestDetails(requestData) {
  ...
  document.getElementById("d-method").className =
    `badge border font-monospace method-${requestData.method}`;
  document.getElementById("d-id").textContent = `Request Id: ${requestData.id}`;
  document.getElementById("d-type").textContent =
    `Body Type: ${requestData.type || "_"}`;
  ...
  headersTable.innerHTML = Object.entries(requestData.headers)
    ...
    .join("");
  document.getElementById("badge-headers").textContent = Object.keys(
    requestData.headers,
  ).length;
